...
import { useGetBudgetsQuery, useDeleteBudgetMutation } from "../../../../api/budgetApi.ts";
import { setPage, setPerPage, setSearch } from "../../../../slice/budgetTableSlice.ts";
...
type SortKey = "unit" | "year" | "quarterly";
...
export default function BudgetDataTable() {
  ...
  const { user } = useAuth();
  const { page, per_page, search } = useSelector((state: RootState) => state.budgetTable);
  const [sortKey, setSortKey] = useState<SortKey>("unit");
  const [sortOrder, setSortOrder] = useState<SortOrder>("asc");
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [selectedBudget, setSelectedBudget] = useState<{ id: number; unit: string } | null>(null);
  ...
  const handleDeleteClick = (budget: { id: number; unit: string }) => {
  ...
  const BudgetYearlyOptions: number[] = [2024, 2025, 2026];
  const QuarterlyOptions: string[] = ["Q1", "Q2", "Q3", "Q4"];

  const meta = budgets?.meta || { total: 0, total_pages: 0 };
      ...
      <div className="overflow-hidden bg-white dark:bg-white/[0.03] rounded-xl">
        <div className="flex flex-col gap-2 px-4 py-4 border border-b-0 border-gray-100 dark:border-white/[0.05] rounded-t-xl sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <Button variant="primary" size="sm" onClick={() => navigate("/budgets/add")}>Add Budget</Button>
            </div>
          </div>
        </div>
                    ...
                    <TableCell className="px-4 py-3 font-normal dark:text-gray-400/90 text-gray-800 border border-gray-100 dark:border-white/[0.05] text-theme-sm whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        <button 
                          ...
                          onClick={() => handleDeleteClick({ id: budget.id ?? 0, unit: budget.unit?.name ?? '' })}
                          title="Delete"
                        >
